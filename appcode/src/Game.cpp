...
void Game::placePieces()
{
    /* White Pieces */
    /* Rooks */
    chessboard[0].placePiece(std::make_unique<Rook>(Color::white));
    chessboard[7].placePiece(std::make_unique<Rook>(Color::white));
    /* Knights */
    chessboard[1].placePiece(std::make_unique<Knigth>(Color::white));
    chessboard[6].placePiece(std::make_unique<Knigth>(Color::white));
    /* Bishops */
    chessboard[2].placePiece(std::make_unique<Bishop>(Color::white));
    chessboard[5].placePiece(std::make_unique<Bishop>(Color::white));
    /* Queen */
    chessboard[3].placePiece(std::make_unique<Queen>(Color::white));
    /* King */
    chessboard[4].placePiece(std::make_unique<King>(Color::white));
    /* Pawns */
    for (int i = 8; i < 16; i++)
    {
        chessboard[i].placePiece(std::make_unique<Pawn>(Color::white));
    }
    /* Black Pieces */
    /* Rooks */
    chessboard[56].placePiece(std::make_unique<Rook>(Color::black));
    chessboard[63].placePiece(std::make_unique<Rook>(Color::black));
    /* Knights */
    chessboard[57].placePiece(std::make_unique<Knigth>(Color::black));
    chessboard[62].placePiece(std::make_unique<Knigth>(Color::black));
    /* Bishops */
    chessboard[58].placePiece(std::make_unique<Bishop>(Color::black));
    chessboard[61].placePiece(std::make_unique<Bishop>(Color::black));
    /* Queen */
    chessboard[59].placePiece(std::make_unique<Queen>(Color::black));
    /* King */
    chessboard[60].placePiece(std::make_unique<King>(Color::black));
    /* Pawns */
    for (int i = 48; i < 56; i++)
    {
        chessboard[i].placePiece(std::make_unique<Pawn>(Color::black));
    }
}
...
void Game::playMove(Coordinate currentCoord, Coordinate targetCoord)
{
    ...
    if (actualMove != end(availableMoves))
    {
        ...
        if ( temp == targetCoord)
        {
            ...
            if (*actualMove == Moves::Pawn && (targetCoord.getRow()==0 && targetCoord.getRow()==7)) Game::replacePawn(targetIdx);
...
void Game::replacePawn(int targetIdx)
{
    auto newPiece = Game::getPawnToReplace();
    /* Remove the Pawn */
    removedPieces.push_back(chessboard[targetIdx].getPiece());
...
std::unique_ptr<Piece> Game::getPawnToReplace(){
    std::string message = (turn == Color::white ? "White" : "Black");
